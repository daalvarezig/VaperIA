...
// POST /data/sales — registrar venta manual (soporta múltiples líneas)
router.post('/sales', async (req, res) => {
  ...
  try {
    ...
    // 2. Líneas + stock por cada línea
    for (const l of lines) {
      const model_id  = l.model.id;
      const flavor_id = l.flavor.flavor_id;
      const unit_cost = l.model.cost;

      const { error: itemErr } = await supabase.from('sale_items').insert({
        sale_id: sale.id, model_id, flavor_id, qty: l.qty, unit_price: l.price, unit_cost,
      });
      ...
      if (inv) {
        await supabase.from('inventory').update({
          stock_units: Math.max(0, inv.stock_units - l.qty),
      ...
      await supabase.from('inventory_movements').insert({
        ...
        movement_type: 'sale', qty_delta: -l.qty,
